...
import { generateAnswer, generateChat, generateQuizTrainerMCQ, scoreQuizTrainerMCQ, generateFlashcards, dialogueStart, dialogueStep, dialogueHint, dialogueFeedback, } from "./groqClient.js";
...
api.post("/upload", async (c) => {
    try {
        ...
        for (const [, v] of Object.entries(body)) {
            const pushIfFile = (x) => {
                if (x && typeof x.arrayBuffer === "function")
                    files.push(x);
            };
            if (Array.isArray(v))
                v.forEach(pushIfFile);
            else
                pushIfFile(v);
        }
        // Parse flags/ids for append/merge
        const scalar = (x) => (Array.isArray(x) ? x[0] : x);
        const appendRaw = scalar(body.append);
        const mergeToRaw = scalar(body.mergeTo) ?? scalar(body.materialId);
        const doAppend = typeof appendRaw === "string"
            ? /^(true|1|yes|on)$/i.test(appendRaw)
            : !!appendRaw;
        const targetId = doAppend ? String(mergeToRaw || "") : "";
        if (!files.length) {
            return c.json({
                error: 'Provide multipart/form-data with one or more "file" fields (PDF/TXT/PNG/JPG/DOCX/PPTX)',
            }, 400);
        }
        ...
        for (const f of files) {
            ...
            if (type.includes("pdf") || name.toLowerCase().endsWith(".pdf")) {
                ...
                if (!text.trim()) {
                    return c.json({ error: `Unable to read text from image "${name}" (OCR empty)` }, 400);
                }
            }
        ...
        if (doAppend) {
            if (!targetId) {
                return c.json({
                    error: "append=true requires 'mergeTo' or 'materialId' to be provided",
                }, 400);
            }
            ...
            try {
                ...
                if (e?.code === "ENOENT") {
                    return c.json({ error: `materialId '${targetId}' not found for append` }, 404);
                }
                throw e;
            }
            const existingBytes = Buffer.byteLength(existing, "utf8");
            if (existingBytes + sizeAdded > LIMIT) {
                return c.json({ error: "Total material size would exceed 10 MB", limit: "10MB" }, 413);
            }
...
api.post("/chat", async (c) => {
    try {
        ...
        const text = materialId || materialText
...
api.post("/quiz/trainer/mcq/start", async (c) => {
    try {
        ...
        console.error(err);
        return c.json({ error: "MCQ trainer start failed", detail: err?.message }, 500);
    }
});
// MCQ Trainer: score selected options and explain + study plan
api.post("/quiz/trainer/mcq/score", async (c) => {
    try {
        const { materialId, materialText, questions, userAnswers } = await c.req.json();
        const text = materialId || materialText
            ? await readMaterial(materialId, materialText)
            : "";
        ...
        console.error(err);
        return c.json({ error: "MCQ trainer score failed", detail: err?.message }, 500);
    }
});
...
api.post("/flashcards", async (c) => {
    try {
        ...
        console.error(err);
        return c.json({ error: "Flashcards generation failed", detail: err?.message }, 500);
    }
});
// ===== Dialogue endpoints =====
api.post("/dialogue/start", async (c) => {
    try {
        ...
        console.error(err);
        return c.json({ error: "Dialogue start failed", detail: err?.message }, 500);
    }
});
api.post("/dialogue/step", async (c) => {
    try {
        const { materialId, materialText, topics, currentTopicIndex, userMessage, lastCoachQuestion, language, } = await c.req.json();
        const text = materialId || materialText
            ? await readMaterial(materialId, materialText)
            : "";
        // Special checkpoint: "How am I doing?"
        const isHowAmIDoing = typeof userMessage === "string" &&
            /^\s*how\s+am\s+i\s+doing\??\s*$/i.test(userMessage);
        if (isHowAmIDoing) {
            const total = Array.isArray(topics) ? topics.length : 3;
            const idx = Math.max(0, Math.min(Number(currentTopicIndex || 0), total - 1));
            const title = (Array.isArray(topics) && topics[idx] && topics[idx].title) ||
                (language === "id" ? "topik saat ini" : "the current topic");
            const completed = Math.max(0, Math.min(idx, total));
            const msg = language === "id"
                ? `Kamu telah menyelesaikan ${completed} dari ${total} topik, dan saat ini kita membahas "${title}". Untuk melanjutkan: ${lastCoachQuestion || "silakan jawab pertanyaan terakhir."}`
                : `You've completed ${completed} of ${total} topics, and we are currently working on "${title}". To pick up where we left off: ${lastCoachQuestion || "please respond to the last question."}`;
            return c.json({
                addressed: false,
                moveToNext: false,
                coachMessage: msg,
        ...
        const currTitle = (Array.isArray(topics) &&
            topics?.[Number(currentTopicIndex || 0)]?.title) ||
            "";
        const nextTitle = (Array.isArray(topics) &&
...
api.post("/dialogue/hint", async (c) => {
    try {
        const { materialId, materialText, currentTopicTitle, language } = await c.req.json();
        const text = materialId || materialText
            ? await readMaterial(materialId, materialText)
            : "";
...
api.post("/dialogue/feedback", async (c) => {
    try {
        const { materialId, materialText, topics, history, language } = await c.req.json();
        const text = materialId || materialText
            ? await readMaterial(materialId, materialText)
            : "";
        ...
        console.error(err);
        return c.json({ error: "Dialogue feedback failed", detail: err?.message }, 500);
    }
});
function escapeRegExp(s) {
    return s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}
function parseFileSegments(text) {
    const fileRe = /(^|\n)===== FILE: ([^\n]+) =====\n/g;
    const appendRe = /(^|\n)===== APPEND BATCH [^\n]* =====\n/g;
...
api.get("/material/:id", async (c) => {
    try {
        ...
        if (!id)
            return c.json({ error: "material id required" }, 400);
...
api.post("/material/:id/remove", async (c) => {
    try {
        ...
        if (!text)
            return c.json({ error: "material not found or empty" }, 404);
